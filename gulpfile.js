...
pipes.orderedVendorScripts = function() {
    return plugins.order(['zepto.js', 'angular.js', 'angular-translate.js']);
};

pipes.orderedAppScripts = function() {
    return plugins.order(['app.js']);
};
...
pipes.validatedPartials = function() {
    return gulp.src(paths.partials)
        .pipe(plugins.htmlhint({'doctype-first': false, 'tag-pair': false}))
...
gulp.task('watch-prod', ['clean-build-app-prod', 'validate-devserver-scripts'], function() {
    ...
    // start nodemon to auto-reload the dev server
    plugins.nodemon({ script: 'server.js', ext: 'js', watch: ['dev/'], env: {NODE_ENV : 'production'} })
        .on('change', ['validate-devserver-scripts'])
        .on('restart', function () {
            console.log('[nodemon] restarted dev server');
        });
